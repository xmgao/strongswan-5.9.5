...
static bool updatepsk(chunk_t key) {
	...
	cr = connect(fd, &serv_addr, sizeof(serv_addr)); //¡¨Ω”km∑˛ŒÒ∆˜
	if (cr < 0) {
	...
	rbuf[ret] = '\0';
	DBG0(DBG_IKE, "quantum key for update psk:%s", rbuf);// π≤œÌ√‹‘ø
	strcat(keyold, key.ptr);					//ÃÌº”µΩƒ©Œ≤
	...
	private_psk_authenticator_t *this, message_t *message)
{
	...
	DBG0(DBG_IKE, "pre-shared key:%s", key->get_key(key).ptr);// π≤œÌ√‹‘ø
